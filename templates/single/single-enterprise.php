...
/**
 * Template Name: 企业详情页
 * Description: 企业官网详情页模板，支持标题跳转链接功能
 */

// 检查是否设置了跳转链接，如果设置了则直接重定向
$title_link = '';
if (have_posts()) {
    the_post();
    $title_link = get_post_meta(get_the_ID(), 'wuchaiwp_enterprise_title_link', true);
    rewind_posts(); // 重置循环
}

// 如果设置了跳转链接，直接重定向
if (!empty($title_link) && !is_admin()) {
    wp_redirect(esc_url($title_link));
    exit;
}
                                ...
                                <span class="current"><a href="<?php echo get_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></span>
                                    ...
                                    <header class="entry-header">
                                        <h1 class="entry-title">
                                            ...
                                            $title_link = get_post_meta(get_the_ID(), 'wuchaiwp_enterprise_title_link', true);
                                            if (!empty($title_link)) {
                                                echo '<a href="' . esc_url($title_link) . '" rel="bookmark" target="_blank">' . get_the_title() . '</a>';
                                            } else {
                                    ...
                                    <!-- 企业页面自定义字段展示 -->
                                    <?php
                                    $page_type = get_post_meta(get_the_ID(), 'wuchaiwp_enterprise_page_type', true);
                                    
                                    if ($page_type == 'case') {
                                        // 案例展示字段
                                        $case_client = get_post_meta(get_the_ID(), 'enterprise_case_client', true) ?: get_post_meta(get_the_ID(), 'wuchaiwp_case_client', true);
                                        $case_industry = get_post_meta(get_the_ID(), 'enterprise_case_industry', true) ?: get_post_meta(get_the_ID(), 'wuchaiwp_case_industry', true);
                                        $case_industry_link = get_post_meta(get_the_ID(), 'enterprise_case_industry_link', true);
                                        $case_icon = get_post_meta(get_the_ID(), 'enterprise_case_icon', true) ?: '📋';
                                        $case_tags = get_post_meta(get_the_ID(), 'enterprise_case_tags', true);
                                        $case_date = get_post_meta(get_the_ID(), 'enterprise_case_date', true);
                                        $case_author = get_post_meta(get_the_ID(), 'enterprise_case_author', true);
                                        
                                        if (!empty($case_client) || !empty($case_industry) || !empty($case_tags) || !empty($case_date) || !empty($case_author)) {
                                            echo '<div class="enterprise-fields case-fields">';
                                            echo '<h3>' . $case_icon . ' 案例信息</h3>';
                                            echo '<div class="fields-grid">';
                                            
                                            if (!empty($case_client)) {
                                                echo '<div class="field-item"><span class="field-label">👥 客户名称</span><span class="field-value">' . esc_html($case_client) . '</span></div>';
                                            }
                                            if (!empty($case_industry)) {
                                                if (!empty($case_industry_link)) {
                                                    echo '<div class="field-item"><span class="field-label">🏭 所属行业</span><span class="field-value"><a href="' . esc_url($case_industry_link) . '" target="_blank">' . esc_html($case_industry) . '</a></span></div>';
                                                } else {
                                                    echo '<div class="field-item"><span class="field-label">🏭 所属行业</span><span class="field-value">' . esc_html($case_industry) . '</span></div>';
                                                }
                                            }
                                            if (!empty($case_tags)) {
                                                $tags = explode(',', $case_tags);
                                                echo '<div class="field-item"><span class="field-label">🏷️ 案例标签</span><span class="field-value tags-wrap">';
                                                foreach ($tags as $tag) {
                                                    echo '<span class="tag">' . trim(esc_html($tag)) . '</span>';
                                                }
                                                echo '</span></div>';
                                            }
                                            if (!empty($case_date)) {
                                                echo '<div class="field-item"><span class="field-label">📅 案例日期</span><span class="field-value">' . esc_html($case_date) . '</span></div>';
                                            }
                                            if (!empty($case_author)) {
                                                echo '<div class="field-item"><span class="field-label">👤 案例作者</span><span class="field-value">' . esc_html($case_author) . '</span></div>';
                                            }
                                        ...
                                        // 产品介绍字段
                                        $product_price = get_post_meta(get_the_ID(), 'enterprise_product_price', true);
                                        $product_version = get_post_meta(get_the_ID(), 'enterprise_product_version', true);
                                        $product_category = get_post_meta(get_the_ID(), 'enterprise_product_category', true);
                                        $product_download = get_post_meta(get_the_ID(), 'enterprise_product_download', true);
                                        $product_features = get_post_meta(get_the_ID(), 'enterprise_product_features', true);
                                        
                                        // 获取全局标题设置
                                        $titles_settings = get_option('wuchaiwp_titles_settings', array());
                                        
                                        // 获取自定义标签（优先文章自定义，其次全局设置，最后默认值）
                                        $price_label = get_post_meta(get_the_ID(), 'enterprise_product_price_label', true) ?: (!empty($titles_settings['product_price_label']) ? $titles_settings['product_price_label'] : '💰 产品价格');
                                        $version_label = get_post_meta(get_the_ID(), 'enterprise_product_version_label', true) ?: (!empty($titles_settings['product_version_label']) ? $titles_settings['product_version_label'] : '🔖 产品版本');
                                        $category_label = get_post_meta(get_the_ID(), 'enterprise_product_category_label', true) ?: (!empty($titles_settings['product_category_label']) ? $titles_settings['product_category_label'] : '📁 产品分类');
                                        $download_label = get_post_meta(get_the_ID(), 'enterprise_product_download_label', true) ?: (!empty($titles_settings['product_download_label']) ? $titles_settings['product_download_label'] : '📥 下载链接');
                                        $features_label = get_post_meta(get_the_ID(), 'enterprise_product_features_label', true) ?: (!empty($titles_settings['product_features_label']) ? $titles_settings['product_features_label'] : '✨ 产品特性');
                                        $download_text = get_post_meta(get_the_ID(), 'enterprise_product_download_text', true) ?: (!empty($titles_settings['product_download_text']) ? $titles_settings['product_download_text'] : '立即下载');
                                        
                                        if (!empty($product_price) || !empty($product_version) || !empty($product_category) || !empty($product_download) || !empty($product_features)) {
                                            echo '<div class="enterprise-fields product-fields">';
                                            echo '<h3>📦 产品信息</h3>';
                                            echo '<div class="fields-grid">';
                                            
                                            if (!empty($product_price)) {
                                                echo '<div class="field-item price-item"><span class="field-label">' . esc_html($price_label) . '</span><span class="field-value">' . esc_html($product_price) . '</span></div>';
                                            }
                                            if (!empty($product_version)) {
                                                echo '<div class="field-item"><span class="field-label">' . esc_html($version_label) . '</span><span class="field-value">' . esc_html($product_version) . '</span></div>';
                                            }
                                            if (!empty($product_category)) {
                                                echo '<div class="field-item"><span class="field-label">' . esc_html($category_label) . '</span><span class="field-value">' . esc_html($product_category) . '</span></div>';
                                            }
                                            if (!empty($product_download)) {
                                                echo '<div class="field-item"><span class="field-label">' . esc_html($download_label) . '</span><span class="field-value"><a href="' . esc_url($product_download) . '" target="_blank" class="download-link">' . esc_html($download_text) . '</a></span></div>';
                                            }
                                            if (!empty($product_features)) {
                                                $features = explode("\n", $product_features);
                                                echo '<div class="field-item features-item"><span class="field-label">' . esc_html($features_label) . '</span><span class="field-value"><ul>';
                                                foreach ($features as $feature) {
                                                    if (!empty(trim($feature))) {
                                                        echo '<li>✓ ' . trim(esc_html($feature)) . '</li>';
                                                    }
                                                }
                                                echo '</ul></span></div>';
                                            }
                                        ...
                                        // 企业简介字段
                                        $company_address = get_post_meta(get_the_ID(), 'enterprise_company_address', true);
                                        $company_phone = get_post_meta(get_the_ID(), 'enterprise_company_phone', true);
                                        $company_email = get_post_meta(get_the_ID(), 'enterprise_company_email', true);
                                        $company_founded = get_post_meta(get_the_ID(), 'enterprise_company_founded', true);
                                        $company_size = get_post_meta(get_the_ID(), 'enterprise_company_size', true);
                                        $company_ceo = get_post_meta(get_the_ID(), 'enterprise_company_ceo', true);
                                        
                                        if (!empty($company_address) || !empty($company_phone) || !empty($company_email) || !empty($company_founded) || !empty($company_size) || !empty($company_ceo)) {
                                            echo '<div class="enterprise-fields about-fields">';
                                            echo '<h3>🏢 企业信息</h3>';
                                            echo '<div class="fields-grid">';
                                            
                                            if (!empty($company_address)) {
                                                echo '<div class="field-item"><span class="field-label">📍 公司地址</span><span class="field-value">' . esc_html($company_address) . '</span></div>';
                                            }
                                            if (!empty($company_phone)) {
                                                echo '<div class="field-item"><span class="field-label">📞 联系电话</span><span class="field-value"><a href="tel:' . esc_attr($company_phone) . '">' . esc_html($company_phone) . '</a></span></div>';
                                            }
                                            if (!empty($company_email)) {
                                                echo '<div class="field-item"><span class="field-label">📧 电子邮箱</span><span class="field-value"><a href="mailto:' . esc_attr($company_email) . '">' . esc_html($company_email) . '</a></span></div>';
                                            }
                                            if (!empty($company_founded)) {
                                                echo '<div class="field-item"><span class="field-label">📅 成立时间</span><span class="field-value">' . esc_html($company_founded) . '</span></div>';
                                            }
                                            if (!empty($company_size)) {
                                                echo '<div class="field-item"><span class="field-label">👥 公司规模</span><span class="field-value">' . esc_html($company_size) . '</span></div>';
                                            }
                                            if (!empty($company_ceo)) {
                                                echo '<div class="field-item"><span class="field-label">👔 法人代表</span><span class="field-value">' . esc_html($company_ceo) . '</span></div>';
                                            }
                                        ...
                                        // 新闻动态字段
                                        $news_author = get_post_meta(get_the_ID(), 'enterprise_news_author', true);
                                        $news_source = get_post_meta(get_the_ID(), 'enterprise_news_source', true);
                                        $news_date = get_post_meta(get_the_ID(), 'enterprise_news_date', true);
                                        $news_category = get_post_meta(get_the_ID(), 'enterprise_news_category', true);
                                        
                                        if (!empty($news_author) || !empty($news_source) || !empty($news_date) || !empty($news_category)) {
                                            echo '<div class="enterprise-fields news-fields">';
                                            echo '<h3>📰 新闻信息</h3>';
                                            echo '<div class="fields-grid">';
                                            
                                            if (!empty($news_author)) {
                                                echo '<div class="field-item"><span class="field-label">👤 作者</span><span class="field-value">' . esc_html($news_author) . '</span></div>';
                                            }
                                            if (!empty($news_source)) {
                                                echo '<div class="field-item"><span class="field-label">📤 来源</span><span class="field-value">' . esc_html($news_source) . '</span></div>';
                                            }
                                            if (!empty($news_date)) {
                                                echo '<div class="field-item"><span class="field-label">📅 发布日期</span><span class="field-value">' . esc_html($news_date) . '</span></div>';
                                            }
                                            if (!empty($news_category)) {
                                                echo '<div class="field-item"><span class="field-label">📁 新闻分类</span><span class="field-value">' . esc_html($news_category) . '</span></div>';
                                            }
                                            
                                            echo '</div>';
                                            echo '</div>';
                                        }
                                    } elseif ($page_type == 'devlog') {
                                        // 开发日志字段
                                        $devlog_version = get_post_meta(get_the_ID(), 'enterprise_devlog_version', true);
                                        $devlog_type = get_post_meta(get_the_ID(), 'enterprise_devlog_type', true);
                                        $devlog_date = get_post_meta(get_the_ID(), 'enterprise_devlog_date', true);
                                        $devlog_changelog = get_post_meta(get_the_ID(), 'enterprise_devlog_changelog', true);
                                        ...
                                        $devlog_type_label = isset($type_labels[$devlog_type]) ? $type_labels[$devlog_type] : $devlog_type;
                                        
                                        if (!empty($devlog_version) || !empty($devlog_type) || !empty($devlog_date) || !empty($devlog_changelog)) {
                                            echo '<div class="enterprise-fields devlog-fields">';
                                            echo '<h3>🔧 开发日志</h3>';
                                            echo '<div class="fields-grid">';
                                            
                                            if (!empty($devlog_version)) {
                                                echo '<div class="field-item version-item"><span class="field-label">🔖 版本号</span><span class="field-value">' . esc_html($devlog_version) . '</span></div>';
                                            }
                                            if (!empty($devlog_type)) {
                                                echo '<div class="field-item"><span class="field-label">📝 更新类型</span><span class="field-value type-badge">' . esc_html($devlog_type_label) . '</span></div>';
                                            }
                                            if (!empty($devlog_date)) {
                                                echo '<div class="field-item"><span class="field-label">📅 更新日期</span><span class="field-value">' . esc_html($devlog_date) . '</span></div>';
                                            }
                                            if (!empty($devlog_changelog)) {
                                                $changelogs = explode("\n", $devlog_changelog);
                                                echo '<div class="field-item changelog-item"><span class="field-label">📋 更新内容</span><span class="field-value"><ul>';
                                                foreach ($changelogs as $changelog) {
                                                    if (!empty(trim($changelog))) {
                                                        echo '<li>• ' . trim(esc_html($changelog)) . '</li>';
                                                    }
                                                }
                                                echo '</ul></span></div>';
                                            }
                                    ...
                                    .price-item .field-value {
                                        font-size: 18px;
                                        font-weight: 600;
                                        color: #e74c3c;
                                    }
                                    
                                    .version-item .field-value {
                                        font-size: 16px;
                                        font-weight: 600;
                                        color: #3498db;
                                    }
                                    ...
                                    // 如果没有手动推荐，根据页面类型显示相关文章
                                    if (!$has_related && !empty($page_type)) {
                                        $related_args = array(
                                            'post_type' => 'page',
                                            'posts_per_page' => 5,
                                            ...
                                            'meta_query' => array(
                                                array(
                                                    'key' => 'wuchaiwp_enterprise_page_type',
                                                    'value' => $page_type
                                                )
                                            )
                                        );
                                        ...
                                        if (!empty($related_posts)) {
                                            ...
                                            $type_label = isset($type_labels[$page_type]) ? $type_labels[$page_type] : '相关';
                                            
                                            echo '<div class="related-posts">';
                                            echo '<h3>📖 相关' . $type_label . '</h3>';
                                            echo '<ul>';
                                            foreach ($related_posts as $related_post) {
                                                echo '<li>';
                                                echo '<a href="' . get_permalink($related_post->ID) . '">' . get_the_title($related_post->ID) . '</a>';
...
<td class="sidebar-cell">
    <aside id="secondary" class="widget-area enterprise-sidebar" role="complementary">
        <div class="enterprise-sidebar-content">
            <?php dynamic_sidebar('enterprise-sidebar'); ?>
        </div>
    </aside>
</td>
...
.enterprise-sidebar-content .sidebar-widget {
    background: #fff;
    border-radius: 10px;
    padding: 24px;
    margin-bottom: 20px;
...
.enterprise-sidebar-content .sidebar-widget:last-child {
    margin-bottom: 0;
}

.enterprise-sidebar-content .widget-title {
    font-size: 16px;
    font-weight: 600;
    color: #333;
    margin: 0 0 15px 0;
...
.enterprise-sidebar .widget-link {
    display: inline-block;
    color: #3498db;
    font-size: 14px;
    text-decoration: none;
...
.enterprise-sidebar .widget-link:hover {
    color: #2980b9;
    text-decoration: underline;
}

/* 快速导航样式 */
.enterprise-sidebar .quick-nav {
    list-style: none;
    padding: 0;
    margin: 0;
}

.enterprise-sidebar .quick-nav li {
    padding: 10px 0;
    border-bottom: 1px dashed #eee;
}

.enterprise-sidebar .quick-nav li:last-child {
    border-bottom: none;
}

.enterprise-sidebar .quick-nav li a {
    color: #333;
    font-size: 14px;
    text-decoration: none;
    transition: color 0.3s ease;
...
.enterprise-sidebar .quick-nav li a:hover {
    color: #3498db;
}

/* 联系信息样式 */
.enterprise-sidebar .contact-info {
    background: transparent !important;
    padding: 0 !important;
}

.enterprise-sidebar .contact-info p {
    display: flex;
...
.enterprise-sidebar .contact-info p:first-child {
    margin-top: 0 !important;
}

.enterprise-sidebar .contact-info p:last-child {
    margin-bottom: 0 !important;
}

.enterprise-sidebar .contact-info a {
    color: #3498db;
    text-decoration: none;
}

.enterprise-sidebar .contact-info a:hover {
    text-decoration: underline;
}
